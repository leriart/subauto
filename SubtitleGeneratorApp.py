...
# modelos de lenguajes soportados
from vosk_models.VOSK_MODELS import VOSK_MODELS
...
class SubtitleGeneratorApp:
        ...
        if self.text_mode.get() == "file":
            ...
            try:
                with open(self.text_path, "r", encoding="utf-8") as f:
                    return TextFormat.clean_text(f.read())
            except:
                return ""
        else:
            return TextFormat.clean_text(self.text_area.get("1.0", tk.END))
    ...
    def validate_srt(content):
        ...
        while i < len(lines):
            ...
            start_time = TextFormat.parse_time(times[0].strip())
            end_time = TextFormat.parse_time(times[1].strip())
    ...
    def write_srt(self, file_path, subtitles):
        ...
        try:
            ...
            for i, sub in enumerate(subtitles, 1):
                ...
                start = TextFormat.format_time(sub['start'])
                end = TextFormat.format_time(sub['end'])
                content += f"{i}\n{start} --> {end}\n{sub['text']}\n\n"
                prev_end = sub['end']
            ...
            while i < len(lines):
                ...
                if lines[i].strip().isdigit():
                    ...
                    if i < len(lines) and '-->' in lines[i]:
                        times = lines[i].split('-->')
                        if len(times) == 2:
                            start = TextFormat.parse_time(times[0].strip())
                            end = TextFormat.parse_time(times[1].strip())
                            i += 1
